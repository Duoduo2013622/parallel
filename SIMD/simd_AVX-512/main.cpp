...
void guass_sse(ofstream &result)
{
    ...
    for(int k=0;k<n;k++)
    {
         __m512 vt = _mm512_set1_ps(A[k][k]); // 存储的四个 float32 都初始化为A[k][k]
        int j;
        for(j=k+1;j+16<=n;j+=16)
        {
            __m512 va= _mm512_loadu_ps(&A[k][j]);
            va= _mm512_div_ps(va,vt);
            _mm512_storeu_ps(&A[k][j], va);// 将 q0 中 4 个 float32，赋值给以 d1 为起始地址的 4 个 float32
...
void guass_sse_duiqi(ofstream &result)//全并行的对齐
{
    long long head,tail,freq ; // timers
    QueryPerformanceFrequency((LARGE_INTEGER*)&freq );
    QueryPerformanceCounter((LARGE_INTEGER *)&head);
...
int main()
{
    ofstream result("C:\\Users\\cuiji\\Desktop\\SIMDX86_AVX-512.txt");
    while(n<1500)
    {
        m_reset(n);
        //cout<<"n="<<n<<":"<<endl;
